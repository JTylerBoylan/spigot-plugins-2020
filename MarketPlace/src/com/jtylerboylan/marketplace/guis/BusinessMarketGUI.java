...
public class BusinessMarketGUI extends UserGUI {
	...
	public BusinessMarketGUI(Player user, Business business, int page) {
		super(user);
	...
	public void clickSlot(int slot) {
		if (slot < 45) {
			if (slot + 1 + (page-1)*45 <= markets_list.size()) {
				UserGUI.openGUI(user, new MarketEditGUI(user, business, markets_list.get(slot+(page-1)*45)));
			}
		}
		if (slot == 51) {
			if (page > 1) {
				UserGUI.openGUI(user, new BusinessMarketGUI(user, business, page-1));
			}
		}
		if (slot == 52) {
			if (markets_list.size() > page*45) {
