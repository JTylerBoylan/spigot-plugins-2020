...
public class BusinessRemoveAccessGUI extends UserGUI {
	...
	public BusinessRemoveAccessGUI(Player user, Business business, int page) {
		super(user);
	...
	public void load() {
		Inventory remove_access_page = Bukkit.createInventory(user, 54, cc(business_prefix + business.getName()));
		
		added = business.getPermitted();
		...
		for (int slot = 0; slot < 45; slot++) {
			...
			if (permit != null)
				...
				try {
					...
					business.unpermit(added.get(slot + (page-1)*45));
	...
	public void clickSlot(int slot) {
		if (slot < 45) {
			if (slot + 1 + (page-1)*45 <= added.size()) {
				...
				business.unpermit(unpermit.getID());
				UserGUI.openGUI(user, new BusinessRemoveAccessGUI(user, business, page));
			}
		}
		if (slot == 51) {
			if (page > 1) {
				UserGUI.openGUI(user, new BusinessRemoveAccessGUI(user, business, page-1));
			}
		}
		if (slot == 52) {
			if (added.size() > page*45) {
