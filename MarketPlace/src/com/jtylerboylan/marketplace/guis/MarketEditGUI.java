...
public class MarketEditGUI extends UserGUI {
	...
	public MarketEditGUI(Player user, Business business, Market market) {
		super(user);
	...
	public void load() {
		...
		Inventory edit_market = Bukkit.createInventory(user, inventory_size, cc(market_prefix + market.getItemID()));
		
		ItemStack delete = UserGUI.tile(Material.REDSTONE_BLOCK, 1, (short) 0, cc("&cDELETE"), cc("&4Click to delete!"));
		...
		edit_market.setItem(0, delete);
		edit_market.setItem(2, item);
		edit_market.setItem(4, sell_multiplier);
		edit_market.setItem(5, min_max);
		edit_market.setItem(6, level);
		edit_market.setItem(8, confirm);
		
		for (int i = 0; i < 8; i++) {
			ItemStack min_price = UserGUI.tile(Material.PAPER, 1, (short) 0, cc("&a" + prices.get(i)), cc("&7Price: &a$" + market.getMinPrice()));
			edit_market.setItem(i+9, min_price);
		}
		
		ItemStack total_min = UserGUI.tile(Material.EMERALD, 1, (short) 0, cc("&7Price: &a$" + market.getMinPrice()), cc("&cClick to reset to $0.00"));
		edit_market.setItem(17, total_min);
		...
		if (inventory_size == 27) {
			for (int i = 0; i < 8; i++) {
				...
				edit_market.setItem(i+18, max_price);
			}
			...
			edit_market.setItem(26, total_max);
		}
		
		user.openInventory(edit_market);
	...
	public void clickSlot(int slot) {
		switch (slot) {
		case 0:
			MarketData.removeMarket(market);
			while(market.getStock() > 64) {
				ItemStack return_item = market.getItemStack().clone();
				return_item.setAmount(64);
				user.getInventory().addItem(return_item);
				market.removeStock(64);
			}				
			ItemStack return_item1 = market.getItemStack().clone();
			return_item1.setAmount(market.getStock());
			user.getInventory().addItem(return_item1);
			market.setStock(0);
			user.sendMessage(cc(prefix + " &aMarket > Market deleted!"));
		...
		case 2:
			...
			if (user.getInventory().getItemInMainHand().getType() == market.getItemStack().getType()) {
				user.sendMessage(cc(prefix + " &cError > You cannot change the item to another of the same type"));
				return;
			}
			while(market.getStock() > 64) {
				ItemStack return_item = market.getItemStack().clone();
			...
			ItemStack return_item = market.getItemStack().clone();
			return_item.setAmount(market.getStock());
			user.getInventory().addItem(return_item);
			market.setStock(0);
			...
			UserGUI.openGUI(user, new MarketEditGUI(user, business, market));
			break;
		case 4:
			int current = 3;
		...
		case 8:
			...
			if (market.getItemStack().getType() == user.getInventory().getItemInMainHand().getType()) {
			...
			user.sendMessage(cc(prefix + " &aMarket > Market edited!"));
			break;
		case 17:
			if (market.getMinPrice() == market.getMaxPrice())
